...
👾 **背景**
在学习了how-to-optimize-gemm之后，很快在实习中用到了矩阵优化的相关知识；以下是具体的背景
- 需要优化的模块实际上有30个连续的循环，矩阵元素都是double类型；
- 30个循环中，分析热点之后，选择第10个循环作为例子，第10个循环实际上是三个矩阵相乘
    - 把其中的矩阵名都换成没有实际意义的$a、b、c$，$mid\_res$指前两个矩阵$a、b$相乘的中间结果，$res$指$mid\_res$和$c$相乘的最终结果；
    - 按照实际场景，矩阵$a$是$60 \times 120$的矩阵，矩阵$b$是$120 \times 60$的矩阵，矩阵$c$是$60 \times 60$的矩阵。在未优化之前，行乘矩阵（ $a, mid\_res$ ）都是按列存储的。
- 模型主要运行的平台架构是ARM64，所以在ARM64方面的优化更多一点。
- 相关代码已传到github
...
时间占比较高的循环：时间占比比较高的是用-----------------标出来的循环，其中第10个循环占比最高，也是主要的优化目标；其余循环占比稍高

时间占比较高的三大原因：

- 没有必要的访存：这些循环的通病是最内层的循环内进行了没有必要的访存
- 矩阵存储不合理：这个问题主要集中在HACK10，后面有具体的分析，主要是按行乘的矩阵按列存、按列存的矩阵按行存（前者需要改进，后者做得比较好）
- 矩阵运算有优化空间：这个问题主要集中在HACK10，后面也会具体分析
...
// 优化后，循环变量改写了（因为原始的不太好理解，这里就是mxk矩阵与kxq矩阵相乘得到的结果矩阵，再与一个qxn的矩阵相乘）
    int m, q, n;
    for (m = 0; m < 60; m++) {
        for (q = 0; q < 60; q++) {
...
由于其他几个循环是比较简单的访存通病，所以这里不再赘述，后面主要写是如何优化的HACK10

## **运算分析**

后面规定都按照$m\times k, k\times q, q \times n$的三个矩阵相乘来表示// 因为原始的形式不好理解

实际上是一个三个矩阵相乘的运算；从这里其实也可以看出矩阵在内存中布局的问题，按行乘的矩阵(也就是 $a$  和 $mid\_res$）是按列存的
...
**因此访存优化完之后，仍需要优化的地方主要有两个：**

- **打包内存不合理的矩阵**
- **矩阵使用intrinsic指令按块计算（其实也是减少了访存）**

后面都是在所有热点循环优化过访存之后的结果，分成两个平台来记录优化过程

## ARM64

### **INTRINSIC——直接计算，lane系列指令**
...
### **INTRINSIC——直接计算，without lane系列指令**
...
### INTRINSIC——PACK MATRIX & CAL 1x8

- **为什么分成1x8的小块？**
    
    ![](https://20231118-1258904223.cos.ap-shanghai.myqcloud.com/image/202602011750820.png)
    
    - 在对应的服务器上，L1 CACHE Line的大小是64字节，正好可以装满8个double
    - 第二个循环本身使用的是一行的res_1，所以按1x8

下面两个小标题对应的分别是使用乘加指令和不使用，不使用会更快；同组另一个实习生告诉我，这应该是因为使用intrinsic指令会有些额外的分支判断，所以不如直接计算来得快（改天自己找下代码看下

**Intrinsic——without lane, maq, st // 在ARM64平台上最快的方案**
...
**Intrinsic——without lane, st; with vmlaq_f64 / vfmaq_f64**
...
第一次计算的时候时间特别长，不知道为什么；后面矩阵计算虽然快，但总的时长比起简单的访存优化并不占优势，推测是因为矩阵到Eigen Matrix的转变（写入写回）很耗时；// 感觉除非把结构体里对应的矩阵类型都变成Eigen的标准矩阵类型
...
其实SEGMM的优化思路，是更好地使用simd；而simd的思路，就是一条指令完成多个计算操作，这样实际上是在减少指令的平均时钟周期数。

在这个过程中，还要让代码配合缓存工作，也就是说，尽量让下次读数的时候别去内存读，因此要把行乘矩阵打包成行存储，列乘矩阵打包成列存储。
